...
function transformToLis(obj){
  let keyValArray = Object.keys(obj);
  return keyValArray.map(item => {
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(arr => {
    return arr.filter(int => typeof int === 'number' && int % 5 === 0).map(filterInt => Math.pow(2,filterInt));
  });
};
...
let findMaleAndFemale = (data) => {
  return data.filter(char => char.gender.includes('male')).map(name => name.name)
    .join(' and ');
};
...
let findShortest = (data) => {
  return data.reduce((char, next) => +char.height < +next.height ? char : next).name;
};
