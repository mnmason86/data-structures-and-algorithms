...
const addOne = (arr) => {
  let oneMore = arr.map( function (n,i) {
    return n + 1;
  });
  return oneMore;
};
...
const forLoopTwoToThe = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++){
    let n = arr[i];
    let twoRaised = Math.pow(2,n);
    raised.push(twoRaised);
  }
  return raised;
};
...
const forEachTwoToThe = (arr) => {
  let raised = [];
  arr.forEach(function(element){
    raised.push(Math.pow(2,element));
  });
  return raised;
};
...
const charCode = (arr) => {
  let codes = arr.map((char,i) => {
    return char.charCodeAt(i * 0);
  });
  return codes;
};
...
const extractAbilities = (arr) => {
  let abilities = arr.map((abty,i) => {
    return abty.ability.name;
  });
  return abilities;
};
