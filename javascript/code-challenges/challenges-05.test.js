...
const addValues = (arr) => {
  let sum = arr.reduce((currentSum, element) => {
...
const addPurchases = (arr) => {
  let costSum = arr.reduce((currentSum, element) => {
    currentSum = element.purchasePrice + currentSum;
    return currentSum;
  }, 0);
  return costSum;
};
...
const countNumberOfElements = (arr) => {
  let arrLength = arr.reduce((arrCount) => {
    return arrCount + 1;
  }, 0);
  return arrLength;
};
...
const returnNames = (arr) => {
  let namesOnly = arr.reduce((charName, element) => {
    charName.push(element.name);
    return charName;
  }, []);
  return namesOnly;
};
